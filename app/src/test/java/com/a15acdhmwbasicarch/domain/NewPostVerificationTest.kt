...
internal class NewPostVerificationTest {
    ...
    fun `check new post for qualification`(): List<DynamicTest> {
        ...
        return listOf(
            ...
            NewPostModel("{${fakeForbiddenWord.toUpperCase()}} title", "normal body")
                    to VerificationStatus.Error(setOf(NewPostErrorType.FORBIDDEN_WORDS_ERROR)),
        ).map { (post, newPostStatus) ->
            DynamicTest.dynamicTest("When post is $post status should by $newPostStatus") {
                newPostVerification.verify(post) shouldBe newPostStatus
            }
        }
    }
}